...
namespace UnityEngine.XR.iOS
{
    public class ARKitRemoteConnection : MonoBehaviour
    {
        ...
        void PreInitCamera()
        {
            ...
            scamera.worldTransform = new UnityARMatrix4x4(new Vector4(1, 0, 0, 0), new Vector4(0, 1, 0, 0),
                new Vector4(0, 0, 1, 0), new Vector4(0, 0, 0, 1));
            Matrix4x4 projMat = Matrix4x4.Perspective(60.0f, 1.33f, 0.1f, 30.0f);
            scamera.projectionMatrix = new UnityARMatrix4x4(projMat.GetColumn(0), projMat.GetColumn(1),
        ...
        void SendInitToPlayer()
        {
            ...
            serializableARSessionConfiguration ssc = new serializableARSessionConfiguration(startAlignment,
                planeDetection, getPointCloud, enableLightEstimation);
            UnityARSessionRunOption roTracking =
                resetTracking ? UnityARSessionRunOption.ARSessionRunOptionResetTracking : 0;
            UnityARSessionRunOption roAnchors = removeExistingAnchors
